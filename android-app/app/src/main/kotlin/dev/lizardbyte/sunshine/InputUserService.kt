...
/**
 * Shizuku UserService running as shell (uid 2000). The shell user holds
 * android.permission.INJECT_EVENTS, so InputManager.injectInputEvent() works here.
 *
 * This class intentionally contains NO references to SunshineNative or libsunshine.so.
 * Loading that library in the Shizuku shell process crashes it (static initializers not
 * designed for a second process). All injection is done via pure Android API reflection.
 */
class InputUserService : IInputBridge.Stub() {
    ...
    override fun injectKey(vkCode: Int, pressed: Boolean) {
        ...
        inject(KeyEvent(now, now, action, keyCode, 0, 0,
    ...
    override fun injectMouseButton(button: Int, pressed: Boolean) {
        ...
        if (pressed) {
            ...
            emit2(now, MotionEvent.ACTION_DOWN, MotionEvent.ACTION_BUTTON_PRESS)
        } else {
            emit2(now, MotionEvent.ACTION_BUTTON_RELEASE,
                if (buttonState and ab.inv() == 0) MotionEvent.ACTION_UP else MotionEvent.ACTION_MOVE)
            buttonState = buttonState and ab.inv()
        }
    }

    private fun emit2(now: Long, a1: Int, a2: Int) {
        val props = arrayOf(MotionEvent.PointerProperties().apply { id = 0; toolType = MotionEvent.TOOL_TYPE_MOUSE })
    ...
    override fun injectScroll(distance: Int) {
        ...
        val e = MotionEvent.obtain(now, now, MotionEvent.ACTION_SCROLL, 1, props, coords,
    ...
    companion object {
        ...
        val vkToKeyCode = mapOf(
            ...
            0x28 to KeyEvent.KEYCODE_DPAD_DOWN, 0x2C to KeyEvent.KEYCODE_SYSRQ,
            0x2D to KeyEvent.KEYCODE_INSERT, 0x2E to KeyEvent.KEYCODE_FORWARD_DEL,
            ...
            0x6A to KeyEvent.KEYCODE_NUMPAD_MULTIPLY, 0x6B to KeyEvent.KEYCODE_NUMPAD_ADD,
            0x6D to KeyEvent.KEYCODE_NUMPAD_SUBTRACT, 0x6E to KeyEvent.KEYCODE_NUMPAD_DOT,
            ...
            0x90 to KeyEvent.KEYCODE_NUM_LOCK, 0x91 to KeyEvent.KEYCODE_SCROLL_LOCK,
